(function(){
	...
	SceneManager.prototype.updateAndRender = function(){
		...
		switch(game.scene){
			case 0:
				...
				game.draw.drawImage(game.R["title"] , (game.canvas.width - 178) / 2 , this.titleY);
				//渲染button
				this.buttonY -= (game.canvas.height - 400) / 20;
				if(this.buttonY < 400) this.buttonY = 400;
				game.draw.drawImage(game.R["button_play"],this.buttonX,this.buttonY);
				//渲染鸟
			...
			case 1:
				...
				game.draw.drawImage(game.R["land"] , 336 , game.canvas.height - 112);
				//渲染鸟
				game.draw.drawImage(game.R["bird0_0"],game.canvas.width * (1-0.618),100);
				//渲染教程
				game.draw.save();
				...
				game.draw.drawImage(game.R["tutorial"],(game.canvas.width   - 114 ) /2,300);
				game.draw.restore();
				break;
			case 2:
				//渲染背景、大地、小鸟
			...
			case 3:
				...
				if(!this.showbomb){
					...
					//爆炸了
					game.draw.drawImage(game.R["baozha" + this.baozha] , game.bird.x - 50, game.bird.y - 100, 100 ,100);
			...
			case 4:
				...
				if(this.showjiangpai){
					game.draw.drawImage(game.R[this.model],(game.canvas.width / 2) - 88,this.score_panelY + 44)
	...
	function scoreRender(){
		...
		for(var i = 0 ; i < score.length ; i++){
			var char = score[i];
			//渲染这一位，基准位置是
			game.draw.drawImage(game.R["shuzi" + char] , baseX + i * 30,100);
		}
	}
})();